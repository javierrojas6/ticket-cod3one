...
const steps = ['LANGUAGE', 'SERVER_REQUIREMENTS', 'DATABASE_CONFIGURATION', 'SETTINGS', 'ADMIN_SETUP', 'COMPLETED'];
        ...
        <Widget className="install-layout">
          <div className="install-layout__header row">
            ...
            <div className="install-layout__header-text  col-md-8">
              <div className="install-layout__header-title">{i18n('INSTALL_HEADER_TITLE')}</div>
              <div className="install-layout__header-description">{i18n('INSTALL_HEADER_DESCRIPTION')}</div>
            </div>
          </div>
          <span className="separator" />
          <div className="install-layout__body row">
            <div className="col-md-3">
              <ul className="install-layout__steps">{steps.map(this.renderStep.bind(this))}</ul>
            </div>
            <div className="install-layout__content col-md-9">{this.props.children}</div>
          </div>
        </Widget>
      </DocumentTitle>
    );
