...
class InstallStep2Requirements extends React.Component {
  ...
  render() {
    return (
      <div className="install-step-2">
        <Header
          title={i18n('STEP_TITLE', { title: i18n('SERVER_REQUIREMENTS'), current: 2, total: 6 })}
          description={i18n('STEP_2_DESCRIPTION')}
        />
        <div className="install-step-2__refresh">
          <Button
            className="install-step-2__refresh-button"
            type="secondary"
            size="medium"
        ...
        <div className="install-step-2__buttons">
          <div className="install-step-2__previous">
            <Button size="medium" onClick={this.onPreviousClick.bind(this)}>
              {i18n('PREVIOUS')}
            </Button>
          </div>
          <div className="install-step-2__next">
            <Button
              disabled={!this.isAllOk()}
              size="medium"
              type="secondary"
              onClick={() => history.push('/install/step-3')}>
              {i18n('NEXT')}
            </Button>
          </div>
        </div>
  ...
  retrieveRequirements() {
    this.setState({ loading: true }, () =>
      API.call({ path: '/system/check-requirements' }).then(({ data }) =>
        this.setState({ requirements: data, loading: false })
      )
    );
  }
}
