...
class InstallStep4UserSystem extends React.Component {
  ...
  render() {
    ...
    return (
      <div className="install-step-4">
        <Header
          title={i18n('STEP_TITLE', { title: i18n('USER_SYSTEM'), current: 4, total: 6 })}
        ...
        <Form onSubmit={this.onSubmit.bind(this)} values={form} onChange={this.onChange.bind(this)}>
          ...
          <div className="install-step-4__buttons">
            <SubmitButton className="install-step-4__next" size="medium" type="secondary">
              {i18n('NEXT')}
            </SubmitButton>
          </div>
        </Form>
  ...
  onSubmit(form) {
    this.props.dispatch(
      ConfigActions.updateUserSystemSettings({
