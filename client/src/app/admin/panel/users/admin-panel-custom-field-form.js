        ...
        <div className="admin-panel-custom-field-form__form-container">
          <Form
            ...
            <FormField
              ...
              fieldProps={{
                size: 'large',
                items: [{ content: i18n('TEXT_INPUT') }, { content: i18n('SELECT_INPUT') }]
              }}
              required
            />
            ...
            <div className="admin-panel-custom-field-form__buttons">
              <Button onClick={this.props.onClose} type="link">
                {i18n('CLOSE')}
              </Button>
  ...
  renderOptionFormFields() {
    return (
      <div className="admin-panel-custom-field-form__options">
        ...
        <Button
          className="admin-panel-custom-field-form__option-add-button"
          iconName="plus"
          size="medium"
          type="secondary"
          onClick={this.onAddOptionClick.bind(this)}
        />
      </div>
    );
  }
  ...
  renderFormOption(option, index) {
    return (
      <div key={index} className="admin-panel-custom-field-form__option">
        <FormField
          className="admin-panel-custom-field-form__option-field"
          name={`option_${index}`}
          label={i18n('OPTION', { index: index + 1 })}
          type="input"
        />
        <Button
          className="admin-panel-custom-field-form__option-delete-button"
          size="medium"
          iconName="times"
          onClick={this.onDeleteOptionClick.bind(this, index)}
        />
      </div>
    );
  }
