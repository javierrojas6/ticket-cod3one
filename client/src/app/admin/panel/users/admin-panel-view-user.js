...
class AdminPanelViewUser extends React.Component {
  ...
  render() {
    return (
      <div className="admin-panel-view-user">
        <Header
          title={i18n('USER_VIEW_TITLE', { userId: this.props.params.userId })}
  ...
  renderNotVerified() {
    return <InfoTooltip className="admin-panel-view-user__unverified" type="warning" text={i18n('UNVERIFIED_EMAIL')} />;
  }

  renderDisabled() {
    return <InfoTooltip className="admin-panel-view-user__unverified" type="warning" text={i18n('USER_DISABLED')} />;
  }
  ...
  retrieveUser() {
    API.call({
      ...
      .then(this.onUserRetrieved.bind(this))
      .catch(() =>
        this.setState({
          invalid: true
        })
