...
class AdminPanelAdvancedSettings extends React.Component {
        ...
        <div className="row">
          ...
          <div className="col-md-12">
            <div className="col-md-4">
              ...
              <FileUploader
                className="admin-panel-advanced-settings__button"
                text="Upload"
                onChange={this.onImportCSV.bind(this)}
              />
            </div>
            <div className="col-md-4">
              ...
              <Button
                ...
                type="secondary"
                size="medium"
                onClick={this.onBackupDatabase.bind(this)}>
                Download
              </Button>
            </div>
            <div className="col-md-4">
              ...
              <Button
                className="admin-panel-advanced-settings__button"
                size="medium"
                onClick={this.onDeleteAllUsers.bind(this)}>
                Delete
              </Button>
            </div>
          </div>
  ...
  renderNoKey() {
    return <div className="admin-panel-advanced-settings__api-keys-none">{i18n('NO_KEY_SELECTED')}</div>;
  }

  renderKey() {
    const { APIKeys, selectedAPIKey } = this.state;
    const { name, token, canCreateTickets, shouldReturnTicketNumber, canCheckTickets, canCreateUser } =
    ...
    return (
      <div className="admin-panel-advanced-settings__api-keys__container-info">
        ...
        <div className="admin-panel-advanced-settings__api-keys__permissions">
          <FormField
            className="admin-panel-advanced-settings__api-keys__permissions__item"
            value={canCreateTickets * 1}
            label={i18n('TICKET_CREATION_PERMISSION')}
            field="checkbox"
          />
          <FormField
            value={shouldReturnTicketNumber * 1}
            label={i18n('TICKET_NUMBER_RETURN_PERMISSION')}
            field="checkbox"
          />
        </div>
        <div className="admin-panel-advanced-settings__api-keys__permissions">
          <FormField
            className="admin-panel-advanced-settings__api-keys__permissions__item"
            value={canCheckTickets * 1}
            label={i18n('TICKET_CHECK_PERMISSION')}
            field="checkbox"
          />
          <FormField value={canCreateUser * 1} label={i18n('USER_CREATION_PERMISSION')} field="checkbox" />
        </div>
        <Button
          className="admin-panel-advanced-settings__api-keys-button"
          size="medium"
          onClick={this.onDeleteKeyClick.bind(this)}>
          {i18n('DELETE')}
        </Button>
      </div>
    );
    ...
    ModalContainer.openModal(
      <Form className="admin-panel-advanced-settings__api-keys-modal" onSubmit={this.addAPIKey.bind(this)}>
        ...
        <div className="admin-panel-advanced-settings__api-keys__buttons-container">
          ...
          <SubmitButton className="admin-panel-advanced-settings__api-keys-modal__submit-button" type="secondary">
            {i18n('SUBMIT')}
          </SubmitButton>
        </div>
      </Form>,
    ...
    return API.call({
      ...
      .catch(() =>
        this.setState({
          ...
          showMessage: true,
          messageTitle: null,
          messageContent: i18n('ERROR_UPDATING_SETTINGS')
        })
      );
  }
        ...
        this.setState({
          ...
          messageContent: result.data.length ? (
            <div>
              {i18n('ERRORS_FOUND')}
              <ul>
                {result.data.map((error, index) => (
                  <li key={`csv-file__key-${index}`}>{error}</li>
                ))}
              </ul>
            </div>
          ) : null
    ...
    return API.call({
      ...
      .then(() =>
        this.setState({
          ...
          showMessage: true,
          messageTitle: null,
          messageContent: i18n('SUCCESS_DELETING_ALL_USERS')
        })
      )
      .catch(() =>
        this.setState({
          ...
          showMessage: true,
          messageTitle: null,
          messageContent: i18n('ERROR_DELETING_ALL_USERS')
        })
      );
  }
