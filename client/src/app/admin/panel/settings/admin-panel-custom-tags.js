...
class AdminPanelCustomTags extends React.Component {
  ...
  renderContent() {
    return (
      <div className="admin-panel-custom-tags__content">
        ...
        <div className="admin-panel-custom-tags__tag-list">{this.props.tags.map(this.renderTag.bind(this))}</div>
      </div>
    );
  }
  ...
  openTagModal() {
    ModalContainer.openModal(<AdminPanelCustomTagsModal onTagCreated={this.retrieveCustomTags.bind(this)} createTag />);
  }

  openEditTagModal(tagId, tagName, tagColor, event) {
    ModalContainer.openModal(
      <AdminPanelCustomTagsModal
        defaultValues={{ name: tagName, color: tagColor }}
        id={tagId}
        onTagChange={this.retrieveCustomTags.bind(this)}
      />
    );
  }
