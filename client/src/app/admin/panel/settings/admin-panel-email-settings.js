...
class AdminPanelEmailSettings extends React.Component {
  ...
  renderEmailSettings() {
    return (
      <div>
        ...
        <Form
          className="admin-panel-email-settings__email-form"
          ...
          <div className="admin-panel-email-settings__email-container">
            <FormField
              className="admin-panel-email-settings__email-server-address"
              name="server-email"
        ...
        <Form
          className="admin-panel-email-settings__image-form"
          ...
          <div className="admin-panel-email-settings__image-container">
            <FormField
              className="admin-panel-email-settings__image-header-url"
              label={i18n('IMAGE_HEADER_URL')}
              name="headerImage"
              required
              infoMessage={i18n('IMAGE_HEADER_DESCRIPTION')}
              fieldProps={{ size: 'large' }}
            />
            <SubmitButton className="admin-panel-email-settings__image-header-submit" type="secondary" size="small">
              {i18n('SAVE')}
            </SubmitButton>
          </div>
        </Form>
        ...
        <div className="admin-panel-email-settings__servers">
          <div className="admin-panel-email-settings__box">
            ...
            <Form
              onSubmit={this.submitSMTP.bind(this)}
              onChange={(smtpForm) => this.setState({ smtpForm })}
              values={this.state.smtpForm}
              loading={this.state.smtpLoading}>
          ...
          <div className="admin-panel-email-settings__box">
            ...
            <Form
              onSubmit={this.submitIMAP.bind(this)}
              onChange={(imapForm) => this.setState({ imapForm })}
              values={this.state.imapForm}
              loading={this.state.imapLoading}>
            ...
            <Message showCloseButton={false} className="admin-panel-email-settings__imap-message" type="info">
              {i18n('IMAP_POLLING_DESCRIPTION', { url: `${runtimeConfig.getApiRoot()}/system/email-polling` })}
            </Message>
          </div>
        </div>
      </div>
  ...
  onItemChange(index, language) {
    if (this.state.edited) {
      AreYouSure.openModal(
        i18n('WILL_LOSE_CHANGES'),
        this.retrieveEmailTemplate.bind(this, index, language || this.state.language)
      );
    } else {
  ...
  submitSMTP(form) {
    ...
    this.editSettings(form, 'SMTP_SUCCESS').then(() =>
      this.setState({
        smtpLoading: false
      })
    );
  ...
  submitIMAP(form) {
    ...
    this.editSettings(form, 'IMAP_SUCCESS').then(() =>
      this.setState({
        imapLoading: false
      })
    );
