...
export function updateSearchTicketsFromURL() {
  ...
  if (currentPath.includes(SEARCH_TICKETS_PATH)) {
    searchTicketsUtils.getFiltersFromParams().then((listConfig) => {
      ...
      if (showFilters !== undefined && currentSearchParams.useInitialValues)
        store.dispatch(searchFiltersActions.changeShowFilters(!showFilters));

      store.dispatch(searchFiltersActions.changeFilters(listConfig));
      store.dispatch(
        searchFiltersActions.retrieveSearchTickets(
          {
...
class AdminPanelSearchTickets extends React.Component {
  ...
  onChangeOrderBy(value) {
    const { listConfig, dispatch } = this.props;
  ...
  onChangeShowFilters() {
    const { showFilters, dispatch } = this.props;
