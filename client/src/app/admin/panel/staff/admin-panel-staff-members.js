    ...
    return (
      <div className="admin-panel-staff-members">
        ...
        <div className="admin-panel-staff-members__wrapper">
          ...
          <Button
            onClick={this.onInviteStaff.bind(this)}
            size="medium"
            type="secondary"
            className="admin-panel-staff-members__button">
            <Icon name="user-plus" className="" /> {i18n('INVITE_STAFF')}
          </Button>
        </div>
        {this.props.loading ? (
          <Loading className="admin-panel-staff-members__loading" backgrounded size="large" />
        ) : (
          <PeopleList
            list={this.getStaffList()}
            page={this.state.page}
            onPageSelect={(index) => this.setState({ page: index + 1 })}
          />
        )}
      </div>
    );
