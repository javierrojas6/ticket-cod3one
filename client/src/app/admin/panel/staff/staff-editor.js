...
class StaffEditor extends React.Component {
  ...
  componentDidMount() {
    ...
    statsUtils
      ...
      .catch((error) => {
        if (runtimeConfig.getShowLogs()) console.error('ERROR: ', error);
      });
  }

  loadStatsRenderer() {
    import('lib-app/stats-renderer')
      .then(({ default: statsRenderer }) => {
        ...
        <div className="row">
          <div className="col-md-4">
            <div className="staff-editor__card">
              <div className="staff-editor__card-data">
                <div className="staff-editor__card-name">{name}</div>
                <div className="staff-editor__card-info">
                  <div className="staff-editor__card-badge">
                    <span className="staff-editor__card-badge-blue">{level}</span>
                    <span className="staff-editor__card-badge-text">{i18n('STAFF_LEVEL')}</span>
                  </div>
                  <div className="staff-editor__card-badge">
                    <span className="staff-editor__card-badge-green">{myData.assignedTickets}</span>
                    <span className="staff-editor__card-badge-text">{i18n('ASSIGNED')}</span>
                  </div>
                  <div className="staff-editor__card-badge">
                    <span className="staff-editor__card-badge-red">{myData.closedTickets}</span>
              ...
              <label className={this.getPictureWrapperClass()}>
                ...
                <input
                  className="staff-editor__image-uploader"
                  type="file"
                  multiple={false}
          ...
          <div className="col-md-8">
            <div className="staff-editor__form">
              <Form
                className="staff-editor__update-email"
                values={{ email: email }}
                ...
                />
                <SubmitButton size="medium" className="staff-editor__submit-button">
                  {i18n('UPDATE_EMAIL')}
                </SubmitButton>
              </Form>
  ...
  renderReInviteStaffButton() {
    const inviteStaffButtonContent = (
      <div>
        <Icon name="user-plus" /> {i18n('INVITE_STAFF')}
      </div>
    );
    ...
    return (
      <div className="staff-editor__staff-invitation-content">
        ...
        <Button
          onClick={this.onReInviteStaffButton.bind(this)}
          size="medium"
          type="secondary"
          className="staff-editor__staff-invitation-button"
          disabled={this.state.loadingReInviteStaff}>
  ...
  renderSendEmailOnNewTicketForm() {
    return (
      <div>
        <span className="separator staff-editor__separator" />
        <Form
          className="staff-editor__update-email-setting"
          ...
          <FormField
            name="sendEmailOnNewTicket"
            label={i18n('SEND_EMAIL_ON_NEW_TICKET')}
            field="checkbox"
            fieldProps={{ size: 'large' }}
          />
          <SubmitButton size="medium" className="staff-editor__submit-button">
  ...
  renderLevelForm() {
    return (
      <div>
        <span className="separator staff-editor__separator" />
        <Form
          className="staff-editor__update-level"
          ...
          <FormField
            ...
            label={i18n('LEVEL')}
            field="select"
            infoMessage={this.getStaffLevelInfo()}
            fieldProps={{
  ...
  renderDepartmentsForm() {
    return (
      <Form
        values={{ departments: this.state.departments }}
    ...
    return (
      <div>
        ...
        <div className="staff-editor__delete">
          <div className="staff-editor__delete-title">{i18n('DELETE_STAFF_MEMBER')}</div>
    ...
    return SessionStore.getDepartments().map((department) => {
      if (department.private * 1) {
        return (
          <span>
            {' '}
            {department.name} <Icon name="user-secret" />{' '}
          </span>
        );
      } else {
