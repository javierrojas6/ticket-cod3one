...
class AdminPanelDepartments extends React.Component {
          ...
          <div className="col-md-8">
            ...
            <Form {...this.getFormProps()}>
              <div className="admin-panel-departments__container">
                <FormField
                  className="admin-panel-departments__container__name"
                  label={i18n('NAME')}
                  name="name"
                  validation="NAME"
                  required
                ...
                <div className="admin-panel-departments__private-option">
                  ...
                  <InfoTooltip
                    className="admin-panel-departments__container__info-tooltip"
              ...
              <div className="admin-panel-departments__buttons-container">
                ...
                <div className="admin-panel-departments__update-name-button__container">
                  <SubmitButton size="medium" className="admin-panel-departments__update-name-button" type="secondary">
                    {formLoading ? <Loading /> : i18n(selectedIndex !== -1 ? 'UPDATE_DEPARTMENT' : 'ADD_DEPARTMENT')}
                  </SubmitButton>
                </div>
              </div>
            </Form>
  ...
  renderDefaultDepartmentForm() {
    ...
    return (
      <div className="admin-panel-departments__default-departments-container">
        ...
        {defaultDepartmentError !== null ? (
          !defaultDepartmentError ? (
            <Message
              ...
              onCloseMessage={this.onCloseMessage.bind(this, 'showSuccessMessage')}
              type="success">
              {i18n('SETTINGS_UPDATED')}
            </Message>
          ) : (
            <Message
              showMessage={showDefaultDepartmentErrorMessage}
              onCloseMessage={this.onCloseMessage.bind(this, 'showDefaultDepartmentErrorMessage')}
              type="error">
        ...
        <Form
          {...this.getDefaultDepartmentFormProps()}
          ...
          <div className="admin-panel-departments__default-departments-container__form__fields">
            ...
            <FormField
              ...
              label={i18n('LOCK_DEPARTMENT_DESCRIPTION')}
              name="locked"
              field="checkbox"
            />
          </div>
  ...
  renderDeleteButton() {
    return (
      <div className="admin-panel-departments__delete-button">
        <Button onClick={this.onDeleteClick.bind(this)} size="medium">
          {i18n('DELETE')}
        </Button>
      </div>
    );
          ...
          content: (
            <span>
              ...
              {department.private * 1 ? (
                <Icon className="admin-panel-departments__private-icon" name="user-secret" />
              ) : null}
              {department.id == defaultDepartmentId ? (
                <spam className="admin-panel-departments__default-icon"> {i18n('DEFAULT')} </spam>
              ) : null}
              {!department.owners ? (
  ...
  getFormProps() {
    ...
    return {
      ...
      loading: formLoading,
      onChange: (form) => {
        this.setState({ form, editedAddDepartmentForm: true });
      },
  ...
  onDefaultDepartmentFormSubmit(formValue) {
    ...
    API.call({
      ...
      .catch((result) =>
        this.setState({
          formLoading: false,
          defaultDepartmentError: result.message,
          showDefaultDepartmentErrorMessage: true
        })
      );
  }
  ...
  deleteDepartment() {
    return API.call({
      ...
      .catch((result) =>
        this.setState({ errorMessage: result.message, showErrorMessage: true, defaultDepartmentError: null })
      );
  }
  ...
  retrieveDepartments(fromUpdateDefaultDepartmentForm = false) {
    ...
    this.setState(
      fromUpdateDefaultDepartmentForm ? { editedDefaultDepartmentForm: false } : { editedAddDepartmentForm: false }
    );
  }
