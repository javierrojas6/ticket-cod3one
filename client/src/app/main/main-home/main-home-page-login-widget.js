...
class MainHomePageLoginWidget extends React.Component {
  ...
  render() {
    return (
      <WidgetTransition
        sideToShow={this.state.sideToShow}
        className={classNames('login-widget__container', this.props.className)}>
        {this.renderLogin()}
      ...
      <Widget className="main-home-page__widget" title={i18n('LOG_IN')} ref="loginWidget">
        <Form {...this.getLoginFormProps()}>
          <div className="login-widget__inputs">
            ...
            />
            <FormField
              placeholder={i18n('PASSWORD_LOWERCASE')}
              name="password"
              className="login-widget__input"
              required
        ...
        <div className="main-home-page__link-buttons-container">
          <Button
            className="login-widget__forgot-password"
            type="link"
  ...
  renderReSendEmailVerificationSection() {
    const { reSendEmailVerificationMessage, reSendEmailVerificationStep, showReSendEmailVerificationMessage } =
      this.state;
    ...
    if (this.props.session.failMessage === 'UNVERIFIED_USER') {
      switch (reSendEmailVerificationStep) {
        case UNVERIFIED_USER_STEP:
          return (
            <Button
              className="login-widget__resend-verification-token"
              type="link"
              onClick={this.onReSendEmailVerificationClick.bind(this)}>
        ...
        case REQUEST_RESULT_STEP:
          return reSendEmailVerificationMessage === 'success' ? (
            <Message
  ...
  renderPasswordRecovery() {
    return (
      <PasswordRecovery
        ...
        recoverSent={this.state.recoverSent}
        formProps={this.getRecoverFormProps()}
        onBackToLoginClick={this.onBackToLoginClick.bind(this)}
      />
    );
  }
  ...
  onForgotPasswordSubmit(formState) {
    ...
    API.call({
      ...
      .then(this.onRecoverPasswordSent.bind(this))
      .catch(this.onRecoverPasswordFail.bind(this));
  }
