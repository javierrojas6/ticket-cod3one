...
class MainLayoutHeader extends React.Component {
  render() {
    return <div className="main-layout-header">{this.renderHeaderOptions()}</div>;
  }
  ...
  renderAccessLinks() {
    const { session, config } = this.props;
    let result;

    if (session.logged) {
      result = (
        <div className="main-layout-header__login-links">
          {i18n('WELCOME')},<span className="main-layout-header__user-name"> {session.userName}</span>
        </div>
      );
    } else {
      result = (
