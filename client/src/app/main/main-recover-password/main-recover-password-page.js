...
class MainRecoverPasswordPage extends React.Component {
      ...
      <div className="main-recover-password-page">
        <Widget
          ...
          <Form
            ...
            onSubmit={this.onRecoverPasswordSubmit.bind(this)}
            loading={this.state.loading}>
            <div className="recover-password__inputs">
              <FormField
                placeholder={i18n('NEW_PASSWORD')}
                name="password"
                className="recover-password__input"
                validation="PASSWORD"
                fieldProps={{ password: true }}
                required
              />
              <FormField
                placeholder={i18n('REPEAT_NEW_PASSWORD')}
  ...
  callRecoverPassword(recoverData) {
    API.call({
      ...
      .then(this.onPasswordRecovered.bind(this))
      .catch(this.onPasswordRecoverFail.bind(this));
  }

  onPasswordRecovered(response = { data: {} }) {
    setTimeout(() => {
      history.push(response.data.staff * 1 ? '/admin' : '/');
    }, 2000);
    this.setState({
      recoverStatus: 'valid',
      showMessage: true,
