...
class DashboardListArticlesPage extends React.Component {
  ...
  renderArticleList() {
    ...
    return <ArticlesList editable={false} articlePath={articlePath} retrieveOnMount={false} />;
  }
    ...
    return (
      <div className="dashboard-list-articles-page__search-result" key={index}>
        <div className="dashboard-list-articles-page__search-result-title">
          <Link to={(this.props.location.pathname == '/articles' ? '/article/' : '/dashboard/article/') + item.id}>
            {item.title}
          </Link>
        </div>
  ...
  onSearch(query) {
    this.setState({
      results: SearchBox.searchQueryInList(
        this.getArticles(),
        query,
...
export default connect((store) => {
  return {
    ...
    config: store.config,
    topics: store.articles.topics.map((topic) => {
