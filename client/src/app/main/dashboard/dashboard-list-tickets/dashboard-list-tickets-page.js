...
class DashboardListTicketsPage extends React.Component {
  ...
  showSupervisorOptions() {
    return (
      <Form
        className="dashboard-ticket-list__supervisor-form"
        values={this.state.form}
        onChange={this.onFormChange.bind(this)}>
        <FormField
          label={i18n('SUPERVISED_USER')}
          name="users"
          field="autocomplete"
          fieldProps={{ items: this.getUserItems() }}></FormField>
        <FormField
          className="dashboard-ticket-list__supervisor-form-checkbox"
          label={i18n('SHOW_MY_TICKETS')}
          name="ownTickets"
          field="checkbox"
        />
      </Form>
