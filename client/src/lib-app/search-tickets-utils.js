...
const getAuthorsForAPI = (authors) => {
  ...
  return JSON.stringify(authorsListFilters.map((author) => ({ id: author.id * 1, isStaff: author.isStaff })));
};
...
export default {
  ...
  getFiltersFromParams(search = window.location.search) {
    ...
    const customTicketsList =
      urlFilters.custom && window.customTicketList && window.customTicketList[urlFilters.custom * 1];
  ...
  getFiltersForURL(filtersWithShouldRemoveParams) {
    const shouldRemoveCustomParam = filtersWithShouldRemoveParams.shouldRemoveCustomParam
      ? filtersWithShouldRemoveParams.shouldRemoveCustomParam
      : false;
    const shouldRemoveUseInitialValuesParam = filtersWithShouldRemoveParams.shouldRemoveUseInitialValuesParam
      ? filtersWithShouldRemoveParams.shouldRemoveUseInitialValuesParam
      : false;
