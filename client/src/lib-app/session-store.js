...
class SessionStore {
  ...
  getItem(key) {
    return this.storage.getItem(runtimeConfig.getRoot() + '_' + key);
  }
  ...
  setItem(key, value) {
    return this.storage.setItem(runtimeConfig.getRoot() + '_' + key, value === undefined ? '' : value);
  }

  removeItem(key) {
    this.storage.removeItem(runtimeConfig.getRoot() + '_' + key);
  }
}
