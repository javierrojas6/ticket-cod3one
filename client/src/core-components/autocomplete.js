...
const ItemsSchema = React.PropTypes.arrayOf(
...
class Autocomplete extends React.Component {
  ...
  componentDidMount() {
    this.throttleItemsQuery = _.throttle(
      (query) => {
  ...
  getUnselectedList(list, selectedList) {
    ...
    return comparerFunction
      ? comparerFunction(list, selectedList)
      : list.filter((item) => !_.some(selectedList, { id: item.id }));
  }
  ...
  onRemoveClick(itemId, isStaffItem, event) {
    const { onChange, onRemoveClick } = this.props;
    const newList = this.getSelectedItems().filter((item) => {
      return isStaffItem !== undefined ? item.isStaff !== isStaffItem || item.id !== itemId : item.id !== itemId;
    });
