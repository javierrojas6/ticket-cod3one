  ...
  getItems() {
    ...
    if (page <= pageRangeDisplayed) {
      totalItems = totalItems.concat(
        _.range(
          1,
          page > pageRangeDisplayed - marginPagesDisplayed + 1
            ? pageRangeDisplayed + 1 + marginPagesDisplayed
      ...
      if (page > pages - pageRangeDisplayed) {
        totalItems = totalItems.concat(
          _.range(
            page < pages - pageRangeDisplayed + marginPagesDisplayed
              ? pages - pageRangeDisplayed - marginPagesDisplayed + 1
              : pages - pageRangeDisplayed + 1,
            pages + 1
          )
        );
      } else {
        totalItems = totalItems.concat(
          _.range(page - Math.floor(pageRangeDisplayed / 2), page + Math.floor(pageRangeDisplayed / 2) + 1)
        );
        totalItems.push('...');
        totalItems = totalItems.concat(_.range(pages - marginPagesDisplayed + 1, pages + 1));
      }
