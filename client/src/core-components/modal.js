...
class Modal extends React.Component {
  ...
  render() {
    return <Motion {...this.getAnimations()}>{this.renderModal.bind(this)}</Motion>;
  }
    ...
    return (
      <div className={this.getClass()} style={{ opacity: animation.fade }} onClick={this.onModalClick.bind(this)}>
        <div
          ...
          onClick={this.onModalContentClick.bind(this)}>
  ...
  renderCloseButton(whiteColor = false) {
    return (
      <span
        className={`modal__close-icon${whiteColor ? ' modal__close-icon__white-color' : ''}`}
  ...
  onModalClick() {
    const { outsideClick, onOutsideClick } = this.props;
