...
class DropDown extends React.Component {
  ...
  renderList({ opacity, translateY }) {
    ...
    return (
      <div className="drop-down__list-container" style={style}>
        {this.props.loading ? (
          <div>
            <Loading className="drop-down__loading" />
          </div>
  ...
  renderCurrentItem() {
    ...
    return (
      <div>
        {iconNode}
        {item.contentOnSelected ? item.contentOnSelected : item.content}
      </div>
    );
  }
  ...
  getKeyActions(event) {
    ...
    const { onHighlightedIndexChange, onMenuToggle } = this.props;
  ...
  onIndexSelected(index) {
    const { onMenuToggle, onHighlightedIndexChange, onChange, loading } = this.props;
