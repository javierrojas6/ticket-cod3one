...
class Table extends React.Component {
    ...
    return (
      <div className={this.getClass()}>
        <table className="table table-responsive">
          <thead>
            <tr className="table__header">{this.props.headers.map(this.renderHeaderColumn.bind(this))}</tr>
          </thead>
          <tbody>{!this.props.loading ? this.getRows().map(this.renderRow.bind(this)) : null}</tbody>
        </table>
  ...
  renderCell(row, key, index) {
    const header = this.props.headers[index];
    let classes = {
      table__cell: true,
      [header.className]: header.className
    };
    ...
    return (
      <td className={classNames(classes)} data-label={this.getHeaderLabel(header)} key={key}>
        {row[key]}
      </td>
    );
  }
  ...
  getHeaderLabel(header) {
    ...
    if (typeof header.value === 'string' || typeof header.value === 'number') {
      return header.value;
    }
  ...
  renderPagination() {
    return this.props.pages || (this.props.pageSize && this.props.rows.length > this.props.pageSize)
