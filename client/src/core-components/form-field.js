...
class FormField extends React.Component {
  ...
  render() {
    ...
    const fieldContent = [this.renderLabel(), this.renderField(), this.renderError()];

    if (this.props.field === 'checkbox') {
      fieldContent.swap(0, 1);
    }

    return <Wrapper className={this.getClass()}>{fieldContent}</Wrapper>;
  }
  ...
  renderError() {
    ...
    if (this.props.error) {
      error = (
        <span className="form-field__error" key="error">
          {this.props.error}
        </span>
      );
    }
  ...
  getFieldProps() {
    const { fieldProps, error, name, placeholder, onBlur, required, field, value, decorator } = this.props;
  ...
  getDivTypes() {
    return ['textarea', 'checkbox-group', 'file', 'date-range'];
  }

  onChange(nativeEvent) {
    const { field, decorator, onChange } = this.props;
