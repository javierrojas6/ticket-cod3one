...
class ToggleList extends React.Component {
  ...
  render() {
    return <div className={this.getClass()}>{this.props.items.map(this.renderItem.bind(this))}</div>;
  }
  ...
  selectItem(index) {
    ...
    _.includes(this.getSelectedList(), index)
      ? _.remove(newSelected, (_index) => _index == index)
