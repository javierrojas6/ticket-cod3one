...
class searchFiltersReducer extends Reducer {
  ...
  onFiltersChange(state, payload) {
    return _.extend({}, state, {
      listConfig: {
        ...
        filters: payload.filters ? _.extend({}, DEFAULT_FILTERS, payload.filtersForAPI) : DEFAULT_FILTERS
      },
      ticketQueryListState: {
        ...state.ticketQueryListState,
        loading: true
  ...
  getListConfig() {
    ...
    if (window.customTicketList && custom && window.customTicketList[custom * 1]) {
      return {
