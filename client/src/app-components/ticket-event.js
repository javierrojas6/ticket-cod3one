...
class TicketEvent extends React.Component {
  static propTypes = {
    type: React.PropTypes.oneOf(['COMMENT', 'ASSIGN', 'UN_ASSIGN', 'CLOSE', 'RE_OPEN', 'DEPARTMENT_CHANGED']),
  ...
  render() {
    ...
    return (
      <div className={this.getClass()}>
        ...
        <div className="col-md-1">{iconNode}</div>
        <div className="col-md-11">{this.renderEventDescription()}</div>
      </div>
    );
  }
  ...
  renderCommentAuthor() {
    ...
    if (level === '3') {
      ...
      commentAuthor = (
        <a className={commentAutorClass} href={VIEW_USER_PATH + author.id}>
          {author.name}
        </a>
      );
    } else {
  ...
  renderEditField() {
    return (
      <Form
        ...
        onChange={(form) => {
          this.setState({ content: form.content });
        }}
        ...
        <div className="ticket-event__submit-edited-comment">
          ...
          <Button size="medium" onClick={this.props.onToggleEdit}>
            {i18n('CLOSE')}
          </Button>
        </div>
      </Form>
  ...
  renderAssignment() {
    ...
    return (
      <div className="ticket-event__circled">
        ...
        <span className="ticket-event__circled-date">
          {' '}
          {i18n('DATE_PREFIX')} {DateTransformer.transformToString(this.props.date)}
        </span>
      </div>
    );
  }
  ...
  renderFooter(file) {
    ...
    if (file) {
      node = (
        <span>
          {' '}
          {this.getFileLink(file)} <Icon name="paperclip" />{' '}
        </span>
      );
    } else {
    ...
    return (
      <div className="ticket-event__items">
        <div className="ticket-event__edited">{edited}</div>
        <div className="ticket-event__file">{node}</div>
      </div>
    );
  }
  ...
  getFileLink(filePath = '') {
    ...
    return <a href={API.getFileLink(filePath)}>{fileName}</a>;
  }
}
