...
class TicketList extends React.Component {
      ...
      <div className="ticket-list">
        <div className="ticket-list__filters">
          <div className="ticket-list__main-filters">
            ...
            {type === 'secondary' && showDepartmentDropdown ? this.renderDepartmentsDropDown() : null}
            {onClosedTicketsShownChange ? this.renderFilterCheckbox() : null}
          </div>
          {showPageSizeDropdown ? (
            <PageSizeDropdown
              className="ticket-list__page-dropdown"
              pages={pages}
              onChange={(event) => this.pageSizeChange(event)}
            />
          ) : null}
        </div>
  ...
  renderMessage() {
    const { message } = queryString.parse(globalThis.location.search);

    switch (message) {
      case 'success':
        return (
          <Message onCloseMessage={this.onCloseMessage} className="create-ticket-form__message" type="success">
            {i18n('TICKET_SENT')}
          </Message>
        );
      case 'fail':
        return (
          <Message onCloseMessage={this.onCloseMessage} className="create-ticket-form__message" type="error">
  ...
  getDepartmentDropdownProps() {
    ...
    return {
      ...
      onChange: (event) => {
        ...
        onDepartmentChange?.(departmentId || null);
      },
      size: 'medium'
    };
  }
  ...
  getTickets() {
    ...
    return selectedDepartment
      ? _.filter(tickets, (ticket) => {
          return ticket.department.id == selectedDepartment;
        })
      : tickets;
  }
  ...
  getTicketTableObject(ticket) {
    ...
    const ticketDateWithOutHoursAndMinutes = Math.floor(
      DateTransformer.UTCDateToLocalNumericDate(JSON.stringify(date * 1)) / 10000
    );
    const stringTicketLocalDateFormat = DateTransformer.transformToString(date, false, true);
    const isRecentTicket = dateTodayWithOutHoursAndMinutes - ticketDateWithOutHoursAndMinutes <= 1;
    const recentDateLabel =
      dateTodayWithOutHoursAndMinutes - ticketDateWithOutHoursAndMinutes ? i18n('YESTERDAY_AT') : i18n('TODAY_AT');
    const ticketDate = isRecentTicket
      ? `${recentDateLabel} ${stringTicketLocalDateFormat.slice(-5)}`
      ...
      title: (
        <div>
          ...
          {(tags || []).map((tagName) => {
            ...
            return <Tag size="small" name={tag?.name} color={tag?.color} key={`${ticketNumber}-${tagName}`} />;
          })}
        </div>
      ),
  ...
  onCloseMessage() {
    history.push(globalThis.location.pathname);
  }
}
