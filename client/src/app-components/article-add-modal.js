  ...
  render() {
    return (
      <div className="article-add-modal">
        <Header
          title={i18n('ADD_ARTICLE')}
          description={i18n('ADD_ARTICLE_DESCRIPTION', { category: this.props.topicName })}
        />
        <Form onSubmit={this.onAddNewArticleFormSubmit.bind(this)} loading={this.state.loading}>
          ...
          />
          <Button
            className="article-add-modal__cancel-button"
            type="link"
            onClick={(event) => {
          ...
          <SubmitButton className="article-add-modal__submit-button" type="secondary">
            {i18n('ADD_ARTICLE')}
          </SubmitButton>
        </Form>
      </div>
