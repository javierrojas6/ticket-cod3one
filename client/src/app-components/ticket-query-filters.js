...
class TicketQueryFilters extends React.Component {
  ...
  render() {
    const { formState, filters, showFilters, ticketQueryListState, staffList } = this.props;
        ...
        <Form
          ...
          <div className="ticket-query-filters__second-row">
            <FormField
              ...
              fieldProps={{
                items: [
                  { content: i18n('LAST_7_DAYS') },
                  { content: i18n('LAST_30_DAYS') },
                  { content: i18n('LAST_90_DAYS') },
                  { content: i18n('LAST_365_DAYS') }
                ],
  ...
  getSelectedDepartments(selectedDepartmentsId) {
    ...
    if (selectedDepartmentsId !== undefined) {
      selectedDepartments = selectedDepartmentsId.map((departmentId) =>
        this.getDepartmentsItems().find((_department) => _department.id === departmentId)
      );
    }
  ...
  getSelectedStaffs(selectedStaffsId) {
    ...
    if (selectedStaffsId !== undefined) {
      selectedStaffs = selectedStaffsId.map((staffId) =>
        ticketUtils.getStaffList({ staffList }, 'toAutocomplete').find((_staff) => _staff.id === staffId)
      );
    }
  ...
  getSelectedTagsName(selectedTagsId) {
    ...
    if (selectedTagsId !== undefined) {
      selectedTagsName = selectedTagsId.map((tagId) => (this.getTags().find((_tag) => _tag.id === tagId) || {}).name);
    }
  ...
  onSubmitListConfig() {
    const { formState, filters, formEdited } = this.props;
  ...
  onChangeForm(data) {
    ...
    const authors = data.authors.map(({ name, id, isStaff, profilePic, color }) => ({
      name,
      id: id * 1,
      isStaff,
      profilePic,
