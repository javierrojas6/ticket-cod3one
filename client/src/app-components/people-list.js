...
class PeopleList extends React.Component {
  ...
  render() {
    const pages = _.range(1, this.getPages() + 1).map((index) => {
      ...
      <div className="people-list">
        ...
        <div className="people-list__pagination">
          <Menu
            type="navigation"
            items={pages}
            selectedIndex={this.props.page - 1}
            onItemClick={this.props.onPageSelect}
            tabbable
          />
        </div>
      </div>
  ...
  getDefaultStyles() {
    return _.times(this.props.pageSize).map(() => {
      return { offset: -100, alpha: 0 };
    });
  }
  ...
  renderList(styles) {
    return <div>{styles.map(this.renderAnimatedItem.bind(this))}</div>;
  }
