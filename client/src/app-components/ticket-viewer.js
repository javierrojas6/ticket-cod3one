...
class TicketViewer extends React.Component {
  ...
  render() {
    ...
    const eventsWithModifiedComments = events.map((event, index) => {
    ...
    return (
      <div className="ticket-viewer__header">
        ...
        <div className="ticket-viewer__edit-title__buttons">
          <Button
            className="ticket-viewer__edit-title__button"
            disabled={this.state.editTitleLoading}
            type="primary"
            size="medium"
            onClick={() => this.setState({ editTitle: false, newTitle: this.props.ticket.title })}>
        ...
        <div className="ticket-viewer__info">
          ...
          <div className="ticket-viewer__info-container-editable">
            <div className="ticket-viewer__info-container-editable__column">
              ...
              <div className="ticket-viewer__info-value">{this.renderOwnerNode()}</div>
            </div>
    ...
    return (
      <div className="ticket-viewer__headers">
        <div className="ticket-viewer__info">
          ...
          <div className="ticket-viewer__info-container">
            <div className="ticket-viewer__info-header">{i18n('TAGS')}</div>
            <div className="ticket-viewer__info-value">{this.renderTags()}</div>
          </div>
        </div>
        <div className="ticket-viewer__info">
          <div className="ticket-viewer__info-container">
          ...
          <div className="ticket-viewer__info-container">
            <div className="ticket-viewer__info-header">{i18n('STATUS')}</div>
            <div className="ticket-viewer__info-value">{i18n(ticket.closed ? 'CLOSED' : 'OPENED')}</div>
          </div>
        </div>
      </div>
    );
  ...
  renderOwnerNode() {
    ...
    if (assignmentAllowed && ticket.owner) {
      ...
      ownerNode = (
        <span className="ticket-viewer__info-owner-name">{ticket.owner ? ticket.owner.name : i18n('NONE')}</span>
      );
    }
  ...
  renderEditedTitleText() {
    return <div className="ticket-viewer__edited-title-text"> {i18n('TITLE_EDITED')} </div>;
  }

  renderCancelButton(option, type) {
    return (
      <Button type="link" size="medium" onClick={() => this.setState({ ['edit' + option]: false })}>
        {i18n(type)}
      </Button>
    );
  }
          ...
          <div className="ticket-viewer__response-field row">
            ...
            <div className="ticket-viewer__response-container">
              ...
              <div className="ticket-viewer__buttons-column">
                <div className="ticket-viewer__buttons-row">{this.renderCloseTicketButton()}</div>
              </div>
            </div>
          </div>
  ...
  renderDeleteTicketButton() {
    return (
      <Button className="ticket-viewer__delete-button" size="medium" onClick={this.onDeleteTicketClick.bind(this)}>
        {i18n('DELETE_TICKET')}
      </Button>
    );
  }
  ...
  renderCloseTicketButton() {
    return (
      <Button size="medium" onClick={this.onCloseTicketClick.bind(this)}>
        {i18n('CLOSE_TICKET')}
      </Button>
    );
  }
  ...
  changeDepartment(departmentId) {
    ...
    return API.call({
      ...
      _.some(userDepartments, { id: departmentId }) || userId === ticket.author.id * 1
  ...
  getStaffAssignmentItems() {
    ...
    let staffAssignmentItems = [{ content: i18n('NONE'), contentOnSelected: i18n('NONE'), id: 0 }];

    staffAssignmentItems = staffAssignmentItems.concat(
      ticketUtils
        .getStaffList({ staffList: staffMembers, ticket }, 'toDropDown')
        .map(({ id, content }) => ({ content, id: id * 1 }))
    );
...
export default connect((store) => {
  return {
    ...
    tags: store.config['tags'].map((tag) => {
      return { ...tag, id: tag.id * 1 };
    })
  };
})(TicketViewer);
